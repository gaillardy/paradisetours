...
<div class="admin-wrapper">
    ...
    <div class="main-content">
        ...
        <section class="content">
            <?php 
                foreach ($sendbox as $item) {
                    ?>
                        <div class="inbox" data-id="<?= $item->id ?>">
                            <div class="widget" data-id="<?= $item->id ?>">
                                <div class="details">
                                    <a href="/nbpt-admin/send-box/<?= $item->id ?>" class="email-link">
                                        <p class="lu"><?= $item->de ?></p>
                                    </a>
                                    <p class="subject"><?= $item->sujet ?></p>
                                    
                                    <div class="meta">
                                        <span class="date" data-received-date="<?= $item->send_at ?>"></span>
                                    </div>
                                </div>
                                <div class="meta">
                                    
                                    <button class="delete-btn" title="Delete" type="button">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        // Récupérer les emails déjà marqués comme lus dans localStorage
        const storedReadEmails = JSON.parse(localStorage.getItem("readEmails")) || [];

        // Initialiser les styles pour les emails déjà marqués comme lus
        document.querySelectorAll(".inbox").forEach((item) => {
            const emailId = item.dataset.id;
            const link = item.querySelector(".email-link");

            // Appliquer le style "lu" si l'email est marqué comme lu
            if (storedReadEmails.includes(emailId)) {
                link.querySelector(".lu").style.color = "black";
            }
        });

        // Ajouter un gestionnaire pour marquer comme lu
        document.querySelectorAll(".email-link").forEach((link) => {
            link.addEventListener("click", function (event) {
                event.preventDefault();
                const parentWidget = this.closest(".inbox");
                const emailId = parentWidget.dataset.id;

                // Marquer l'email comme lu (changer la couleur)
                this.querySelector(".lu").style.color = "black";
                ...
                // Optionnel : Rediriger après un léger délai
                setTimeout(() => {
        ...
        document.querySelectorAll(".delete-btn").forEach((button) => {
            button.addEventListener("click", function () {
                const emailId = this.closest(".widget").dataset.id;
                const parentWidget = this.closest(".inbox");

                if (confirm("Êtes-vous sûr de vouloir supprimer cet email ?")) {
                    fetch(`/nbpt-admin/send-box/delete/${emailId}`, {
                        method: "DELETE",
                    })
                        .then((response) => {
                            if (response.ok) {
    ...
    const inboxItems = Array.from(contentContainer.querySelectorAll(".inbox"));
    ...
    function createPaginationControls() {
        ...
        contentContainer.after(paginationContainer);
        updatePaginationControls();
    }
